...
void vector_swap(vector<int> &v1, vector<int> &v2, int start1, int end1, int start2, int end2)
{
    for(int i = end2-1; i >= start2; i--) {
        v1.insert(v1.begin() + start1,v2[i]);
    }
    for(int i = end1-1+(end2-start2); i >= start1+(end2-start2); i--) {
        v2.insert(v2.begin() + start2,v1[i]);
    }
    for(int i = 0; i<end1-start1; i++){
        v1.erase(v1.begin() + start1 + (end2-start2));
    }
    for(int i = 0; i<end2-start2; i++){
        v2.erase(v2.begin() + start2 + (end1-start1));
    }
}
int main()
{
